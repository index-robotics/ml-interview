...
class Kinematics:
    def __init__(self, xml_path_or_model: str | mujoco.MjModel, site_name: Optional[str]):
        """Initialize the Kinematics object.

        Args:
            xml_path_or_model (str | mujoco.MjModel): Path to the MuJoCo XML model file or the model itself.
            site_name (Optional[str]): Name of the site for which to compute the forward kinematics.
        """
        if isinstance(xml_path_or_model, str):
            model = mujoco.MjModel.from_xml_path(xml_path_or_model)
        else:
            model = xml_path_or_model
